...
상태 관리를 구현하면서 몇 가지 문제에 직면했습니다. <br>
특히, 상태는 CSR(Client-Side Rendering)에서 활용할 수 있다는 점을 파악하게 되었고,<br> 
이를 효율적으로 관리하기 위해 프로젝트의 구조를 고민해야 했습니다.<br>
상태 관리를 효과적으로 사용하기 위해, 먼저 프로젝트의 레이아웃 구조를 설계했습니다.<br> 
이 과정에서 어떤 상태가 전역적으로 관리되어야 하는지, 그리고 어떤 상태가 개별 컴포넌트에 한정되는지를 명확히 정의했습니다.
